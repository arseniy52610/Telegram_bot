...
async def start(message: Message, command: CommandObject, state: FSMContext):
    ...
    if game:
        ...
        await message.answer("Добро пожаловать в\nМорской бой! 🚢☠️\n\nГотовы разбить флот противника, потопив все его корабли? Бросьте вызов своему другу в Telegram или сыграйте со случайным соперником. Всего одна победа – и вы станете матросом! 🎯\n\n<b>Чтобы начать, нажмите на одну из кнопок ниже:</b>", parse_mode='HTML', reply_markup=await menu_keyboard())
        


@router.callback_query(F.data.startswith("game"))
async def play(callback: types.CallbackQuery):
    await callback.message.edit_text("<b>Выберите режим игры 👇</b>", parse_mode='HTML',reply_markup=await game_mode_keyboard())


@router.callback_query(F.data.startswith("profil"))
...
🏆 Рейтинг: <b>{user_info[3]} очков</b>""", parse_mode="HTML", reply_markup=await back_keyboard())
...
async def top(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=await back_keyboard())
...
async def play(callback: types.CallbackQuery):
    await callback.message.edit_text("Добро пожаловать в\nМорской бой! 🚢☠️\n\nГотовы разбить флот противника, потопив все его корабли? Бросьте вызов своему другу в Telegram или сыграйте со случайным соперником. Всего одна победа – и вы станете матросом! 🎯\n\n<b>Чтобы начать, нажмите на одну из кнопок ниже:</b>", parse_mode='HTML', reply_markup=await menu_keyboard())
